...
const StatusNotification = (props) => {
  ...
  var fields1 = [{ key: '통신코드', _classes: 'Tablefirst' }, { key: '충전기아이디', _classes: 'Tablefirst' }, { key: '충전건번호', _classes: 'Tablefirst' }, { key: '상태정보', _classes: 'Tablefirst' }, { key: '로그기록일', _classes: 'Tablefirst' }]
  if (isMobile) { fields1 = [{ key: '통신코드', _classes: 'Tablefirst' }, { key: '충전기아이디', _classes: 'Tablefirst' }, { key: '상태정보', _classes: 'Tablefirst' }] }
  var fields2 = [{ key: '통신코드', _classes: 'Tablefirst' }, { key: '충전기아이디', _classes: 'Tablefirst' }, { key: '충전건번호', _classes: 'Tablefirst' }, { key: '상태정보', _classes: 'Tablefirst' }, { key: '로그기록일', _classes: 'Tablefirst' }]
  if (isMobile) { fields2 = [{ key: '통신코드', _classes: 'Tablefirst' }, { key: '충전기아이디', _classes: 'Tablefirst' }, { key: '상태정보', _classes: 'Tablefirst' }] }
  ...
  useEffect(() => {
    ...
    // 리스트 출력 기본 세팅값 정의 
    axios.get(process.env.REACT_APP_APISERVER + "/api/data/statusnoti/" + tableFilterValue + "/" + page, configHeader).then(result => {
            ...
            <CCard id="listtable2" className="p-5" style={{ width: "100%", position: "relative", float: "left" }}>
              ...
              <CDataTable
                ...
                scopedSlots={{
                  ...
                  '상태정보': (item) => (<td style={{ width: "20%" }}>{item.status}</td>),
                  '로그기록일': (item) => (<td style={{ width: "30%" }}><Moment local format="YYYY-MM-DD  HH:mm">{item.localtime}</Moment></td>),
                }}
              />
              <CPagination
                pages={pages}
