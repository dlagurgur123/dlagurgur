...
const Logininfo = (props) => {
  ...
  var fields1 = [{ key: '통신코드', _classes: 'Tablefirst' }, { key: '카드번호', _classes: 'Tablefirst' }, { key: '충전기아이디', _classes: 'Tablefirst' }, { key: '충전건번호', _classes: 'Tablefirst' }, { key: '등록일시', _classes: 'Tablefirst' }]
  if (isMobile) { fields1 = [{ key: '통신코드', _classes: 'Tablefirst' }, { key: '카드번호', _classes: 'Tablefirst' }, { key: '충전기아이디', _classes: 'Tablefirst' }, { key: '충전건번호', _classes: 'Tablefirst' }, { key: '등록일시', _classes: 'Tablefirst' }] }
  var fields2 = [{ key: '통신코드', _classes: 'Tablefirst' }, { key: '카드번호', _classes: 'Tablefirst' }, { key: '충전기아이디', _classes: 'Tablefirst' }, { key: '충전건번호', _classes: 'Tablefirst' }, { key: '등록일시', _classes: 'Tablefirst' }]
  if (isMobile) { fields2 = [{ key: '통신코드', _classes: 'Tablefirst' }, { key: '카드번호', _classes: 'Tablefirst' }, { key: '충전기아이디', _classes: 'Tablefirst' }, { key: '충전건번호', _classes: 'Tablefirst' }, { key: '등록일시', _classes: 'Tablefirst' }] }
  ...
  useEffect(() => {
    ...
    // 리스트 출력 기본 세팅값 정의 
    axios.get(process.env.REACT_APP_APISERVER + "/api/data/authorize/" + tableFilterValue + "/" + page, configHeader).then(result => {
          ...
          <CRow>
            ...
            <CCard id="listtable2" className="p-5" style={{ width: "100%", position: "relative", float: "left" }}>
              ...
              <CDataTable
                ...
                scopedSlots={{
                  '통신코드': (item) => (<td>{item.pid}</td>),
                  '카드번호': (item) => (<td>{item.idTag} </td>),
                  '충전기아이디': (item) => (<td>{item.cpId}</td>),
                  '충전건번호': (item) => (<td>{item.connectorId}</td>),
                  '등록일시': (item) => (<td> <Moment local format="YYYY-MM-DD  HH:mm">{item.localtime}</Moment></td>),
                }}
              />
              <CPagination
                pages={pages}
